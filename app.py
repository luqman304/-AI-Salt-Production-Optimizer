...
import io, os, numpy as np, pandas as pd, streamlit as st, matplotlib.pyplot as plt
...
st.set_page_config(page_title="AI-Salt-Production-Optimizer", page_icon=":salt:", layout="wide")
...
with st.sidebar:
    ...
    st.markdown("**NASCON FY2024 (public filings):**\n"
                "- Revenue: approx NGN 120.4B\n"
                "- Gross margin: approx 46 percent\n"
                "- EBITDA margin: approx 23 percent\n\n"
                "Sources: NGX audited results & FY2024 investor materials.")
    section = st.radio("Navigate", ["Predictive Maintenance","Energy Optimization","Quality Control (CV)","Logistics Optimization","ROI Calculator"])
...
if section == "Predictive Maintenance":
    ...
    with c1: st.dataframe(df.tail(), use_container_width=True)
    with c2:
        fig, ax = plt.subplots()
        ax.plot(df["date"], df["temp_c"], label="Temp (C)")
        ax2 = ax.twinx(); ax2.plot(df["date"], df["vibration_g"], color="gray", alpha=0.6, label="Vibration (g)")
        ax.set_title("Temperature & Vibration — last 120 days")
        st.pyplot(fig)
    X = df[["temp_c","vibration_g","run_hours"]]; y = df["failure_next_7d"]
    ...
    fig2, ax = plt.subplots(); ax.plot(df["date"], (1/(1+np.exp(-risk_series))).fillna(0))
    ax.set_ylim(0,1); ax.set_title("Failure risk trend (7-day) — demo"); st.pyplot(fig2)
    ...
    c1,c2 = st.columns([1,1])
    with c1: st.dataframe(edf.tail(), use_container_width=True)
    with c2:
        fig, ax = plt.subplots(); ax.plot(edf["date"], edf["energy_mwh"]); ax.set_title("Daily energy (365d)"); st.pyplot(fig)
    ...
    ax.legend(); ax.set_title("Baseline forecast band"); st.pyplot(fig2)
    ...
    if up is None:
        cols = st.columns(4)
        samples = ["sample_data/quality/good1.jpg","sample_data/quality/good2.jpg","sample_data/quality/defect1.jpg","sample_data/quality/defect2.jpg"]
        for i,p in enumerate(samples):
            with cols[i]: st.image(p, caption=os.path.basename(p), use_container_width=True)
    else:
        img = Image.open(up).convert("RGB"); arr = np.asarray(img).astype(np.float32)
        dark = (arr.mean(axis=2) < 120).mean()
        yellow = ((arr[:,:,0]+arr[:,:,1])/2 - arr[:,:,2] > 25).mean()
        score = 0.6*dark + 0.4*yellow; label = "DEFECT" if score>0.08 else "GOOD"
        st.image(img, caption=f"{label} (score={score:.3f})", use_container_width=True)
    fig, ax = plt.subplots(); ax.bar(["GOOD","DEFECT"], [2,2]); ax.set_title("Recent QC outcomes (sample)"); st.pyplot(fig)

elif section == "Logistics Optimization":
    st.subheader("Logistics — Simple Packing Heuristic (Demo)")
    ldf = pd.read_csv("sample_data/logistics.csv"); st.dataframe(ldf.head(), use_container_width=True)
    trucks = st.slider("Available trucks", 3, 12, 6)
    ldf_sorted = ldf.sort_values("distance_km", ascending=False).reset_index(drop=True)
    routes = [[] for _ in range(trucks)]; loads = [0]*trucks
        ...
        if loads[idx] + r["weight_tons"] > 40:
            ...
            for j in range(trucks):
                if loads[j] + r["weight_tons"] <= 40:
                    routes[j].append(r["order_id"]); loads[j]+=r["weight_tons"]; placed=True; break
            if not placed: routes[idx].append(r["order_id"]); loads[idx]+=r["weight_tons"]
        else:
            routes[idx].append(r["order_id"]); loads[idx]+=r["weight_tons"]
    c1,c2 = st.columns([1,1])
    with c1:
        for i,(rt,ld) in enumerate(zip(routes,loads),start=1):
            card(f"Truck {i}", f"Load: **{ld:.1f} tons** • Orders: {', '.join(rt) if rt else '-'}")
    with c2:
        fig, ax = plt.subplots(); ax.scatter(ldf["distance_km"], ldf["cost_ngn"])
        ax.set_xlabel("Distance (km)"); ax.set_ylabel("Cost (NGN)"); ax.set_title("Distance vs Cost"); st.pyplot(fig)
    ...
    gross_profit = rev*(gm/100); ebitda = rev*(ebitda_m/100); cogs = rev-gross_profit; waste_cost = cogs*(waste_pct/100)
    downtime_savings = ebitda*(dt_reduc/100)*0.2
    maint_savings = maint_cost*(maint_reduc/100)
    energy_savings = energy_cost*(energy_reduc/100)
    logistics_savings = logistics_cost*(trans_reduc/100)
    waste_savings = waste_cost*(waste_reduc/100)
    total_savings = downtime_savings + maint_savings + energy_savings + logistics_savings + waste_savings
    ebitda_uplift_pp = (total_savings/rev)*100
    ...
    after = [maint_cost*(1-maint_reduc/100), energy_cost*(1-energy_reduc/100), logistics_cost*(1-trans_reduc/100), waste_cost*(1-waste_reduc/100)]
    fig, ax = plt.subplots(); x = np.arange(len(labels))
    ax.bar(x-0.2, before, width=0.4, label="Before"); ax.bar(x+0.2, after, width=0.4, label="After")
    ax.set_xticks(x); ax.set_xticklabels(labels, rotation=10); ax.legend(); ax.set_ylabel("NGN (annual)"); st.pyplot(fig)
    st.markdown("### Savings Breakdown")
    parts = [("Downtime", downtime_savings), ("Maintenance", maint_savings), ("Energy", energy_savings), ("Logistics", logistics_savings), ("Waste", waste_savings)]
    labels2, sizes2 = zip(*parts); fig2, ax2 = plt.subplots()
    ax2.pie(sizes2, labels=labels2, autopct="%1.1f%%", startangle=140); ax2.axis("equal"); st.pyplot(fig2)
    st.markdown("---")
    if st.button("Export 1-page PDF"):
        buf = io.BytesIO(); c = canvas.Canvas(buf, pagesize=A4); w,h = A4; y=h-2*cm
        c.setFont("Helvetica-Bold", 16); c.drawString(2*cm,y,"AI-Salt-Production-Optimizer — ROI Summary"); y-=1.2*cm
        c.setFont("Helvetica",10); c.drawString(2*cm,y,f"Revenue: NGN {rev:,.0f} | GM: {gm:.1f}% | EBITDA: {ebitda_m:.1f}%"); y-=0.6*cm
        c.drawString(2*cm,y,f"Savings: NGN {total_savings:,.0f} | EBITDA uplift: {ebitda_uplift_pp:.2f} pp | Payback: {payback_months:.1f} mo"); y-=0.8*cm
        c.setFont("Helvetica-Bold",12); c.drawString(2*cm,y,"Savings by Area"); y-=0.6*cm
        for name,val in parts: c.drawString(2.2*cm,y,f"- {name}: NGN {val:,.0f}"); y-=0.5*cm
        y-=0.4*cm; c.setFont("Helvetica-Oblique",9)
        c.drawString(2*cm,y,"Sources: NASCON FY2024 audited results & investor materials (public filings).")
        y-=0.5*cm; c.drawString(2*cm,y,"Demo assumptions; replace with plant data & validated finance inputs.")
